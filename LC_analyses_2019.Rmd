...
#work out proportion of naha against inundated ha for examining 
Data_MLW$propnaha <- Data_MLE$naha/Data_MLE$inha*100
Data_MLE$date <- as.Date(Data_MLE$date, "%Y-%m-%d")
Data_MLE$nahause <- Data_MLE$naha
Data_MLE$naha_include <- "no"
write.csv(Data_MLE, "Data_MLE.csv")
#change naha_include to 'yes' for selected dates
Data_MLE$naha_include[Data_MLE$date %in% as.Date(c("15/03/2013","11/04/2011","26/03/2011","10/03/2011","04/03/2009"), "%d/%m/%Y")] <- "yes"
...
#create 3 ggplots - one for each rainfall station and then print using grid.arrange
MLC <- ggplot () +
  geom_line(data = Data_MLC, aes(x = date, y = comha), color = "blue", size = 0.5) + theme(axis.title.x = element_blank()) + ylab("Area Inundated")  + theme(axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0), size=9)) + labs(title = "Central area of main lake") + theme(plot.title = element_text(size = 9))
MLW <- ggplot () +
  geom_line(data = Data_MLW, aes(x = date, y = comha), color = "blue", size = 0.5) + theme(axis.title.x = element_blank()) + ylab("Area Inundated")  + theme(axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0), size=9)) + labs(title = "Western area of main lake") + theme(plot.title = element_text(size = 9))
P3 <- ggplot () +
  geom_line(data = Data_P3, aes(x = date, y = comha), color = "red", size = 0.5) + theme(axis.title.x = element_blank()) + ylab("Area Inundated") + theme(axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0), size=9))+ labs(title = "Peripheral wetland 3") + theme(plot.title = element_text(size = 9))
P4 <- ggplot () +
  geom_line(data = Data_P4, aes(x = date, y = inha), color = "green", size = 0.5) + ylab("Area Inundated") + theme(axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0), size=9)) + labs(title = "Peripheral wetland 4 on dune") + theme(plot.title = element_text(size = 9))
#arrange all three plots together
grid.arrange(MLC, MLW, P3, P4, ncol=1)
```
